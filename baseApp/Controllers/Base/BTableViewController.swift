...
extension BTableViewController: YopterTextFieldDelegate {
//
...
//
//
    @objc func yopterTextFieldChange(textField: UITextField, indexPath: IndexPath, text: String) {
        let item = tableItems.getItem(section: indexPath.section, at: indexPath.row)
        ...
        BaseDelegate?.BTableView?(tableItems: tableItems, textFieldDidChange: item.value, indexPath: indexPath)
    }
}
//
...
extension BTableViewController: botonDelegate {
    ...
    func botonPressed(indexPath: IndexPath) {
        BaseDelegate?.BTableView?(tableItems: tableItems, buttonPressedAt: indexPath)
    }
}
//
